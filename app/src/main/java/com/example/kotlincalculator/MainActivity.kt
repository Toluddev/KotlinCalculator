...
class MainActivity : AppCompatActivity() {
    ...
    private fun calculateResults(): String {
        ...
        val timesDivision = handleMultiplicationAndDivision(digitsOperators)
        if (timesDivision.isEmpty()) return ""

        val result = handleAdditionAndSubtraction(timesDivision)
    ...
    private fun handleMultiplicationAndDivision(passedList: MutableList<Any>): MutableList<Any> {
        ...
        while (list.contains('x') || list.contains('/')) {
            list = calculateTimesOrDiv(list)
        }
        return list
    }

    private fun calculateTimesOrDiv(passedList: MutableList<Any>): MutableList<Any> {
                ...
                when (operator) {
                    ...
                    '/' -> {
                        if (nextDigit != 0f) {
                            ...
                            return mutableListOf("Error".toFloat())
                        }
                        restartIndex = i + 1
                    }
                    else -> {
